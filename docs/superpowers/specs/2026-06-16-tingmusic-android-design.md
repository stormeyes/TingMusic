...
| `/files/<相对路径>` | GET | 文件原始字节(`application/octet-stream`)。axum `/files/{*path}` 通配捕获、URL 编码;自定义 handler 按**当前曲库根**做路径安全校验。 |

`/files/{*path}` 的 `path` 是相对曲库根的相对路径(各段 URL 编码)。handler 读取运行时的曲库根(随用户改曲库而变),拒绝 `..`/绝对路径,`canonicalize` 后必须仍在根目录内,否则 403/404。**v1 不做 HTTP Range**:下载以整文件为粒度,失败的文件下次 diff 时整体重下。
...
3. 对下载集:`GET /files/<path>` 写入 `…/files/Music/<path>`(按需建子目录)。整文件失败下次 diff 重下(**文件级续传**,不做字节级 Range)。
...
### 4.5 实现注记(Plan 1 已落地,Plan 2 安卓端必须遵守)

Mac 同步服务已实现并真机验证(mDNS 广播 + `/manifest` + `/files/{*path}` + 防穿越 + 开关)。安卓端对接时:

- **URL 编码按段做**:`/files/<path>` 里 `<path>` 的**每个路径段**要 percent-encode(`#`/`?`/空格/中日文等),但**不要**编码 `/` 分隔符。manifest 里给的是**原始**相对路径(如 `群青 - YOASOBI.flac`、`Song#hash.mp3`);若直接字符串拼接,`#` 会被当成 fragment、`?` 会开 query,服务端收不到 → 403/404。等价于对每段做 `encodeURIComponent`。
- **不要依赖 Content-Type**:所有文件都以 `application/octet-stream` 返回。安卓判断"这是歌词文件"靠 manifest 里该项的 `.lrc` 扩展名,而非响应头。
- **端口**:默认 `8737`,但可能顺延;**以 mDNS TXT 的 `port` 字段为准**(已验证 TXT 含 `v=1 lib=<名> port=<实际端口>`)。
- **主机地址**:mDNS 用合成主机名(如 `tingmusic-8737.local.`),`enable_addr_auto` 已把它的 A 记录指向本机真实局域网 IP(实测 `192.168.5.139`);安卓 `NsdManager` resolve 后拿到的 host/IP 可直连。
...
- 新增 `src-tauri/src/sync_server.rs`:
  - `axum` Router:`/manifest`(自定义 handler,遍历当前曲库根,过滤音频 + `.lrc`,产出 4.3 的 JSON)+ `/files/{*path}`(自定义 handler,读取当前曲库根 + 路径安全校验,整文件返回;v1 不做 Range)。
...
- `src-tauri/Cargo.toml`:新增 `axum`、`mdns-sd`、`local-ip-address`(取本机局域网 IP 展示),并给 `tokio` 加 `net` feature。均纯 Rust,不破坏跨平台编译。
